...
class WebRoutesTests(unittest.TestCase):
    ...
    def test_ingest_template_has_single_file_input(self) -> None:
        ...
        self.assertIn('name="title"', ingest)
        self.assertIn('name="author"', ingest)
        self.assertIn("data-txt-title-input", ingest)
        self.assertIn("data-txt-author-input", ingest)
        self.assertIn("data-txt-meta-fields", ingest)
        self.assertIn("txtMetaFields.classList.remove('hidden')", ingest)
        self.assertIn("txtMetaFields.classList.add('hidden')", ingest)
        self.assertIn("TXT 可选覆盖", ingest)
        self.assertIn("data-dedupe-confirm", ingest)
        self.assertIn("data-dedupe-duplicate-list", ingest)
        self.assertIn("data-dedupe-keep-checkbox", ingest)
        self.assertIn("syncTxtMetadataFromPreview", ingest)
        self.assertNotIn("data-txt-meta-modal", ingest)
        self.assertNotIn("data-open-txt-meta-modal", ingest)
        ...
        self.assertIn("syncDedupeConfirm", ingest)
        ...
        self.assertIn("const keepDuplicate = hasDuplicateHit && dedupeKeepCheckbox instanceof HTMLInputElement && dedupeKeepCheckbox.checked;", ingest)
        self.assertIn("dedupeModeInput.value = keepDuplicate ? 'keep' : (hasDuplicateHit ? 'normalize' : 'keep');", ingest)
        self.assertNotIn("window.confirm(", ingest)
    ...
    def test_ingest_preview_template_exposes_structured_preview_metadata(self) -> None:
        root = Path(__file__).resolve().parent.parent
        tpl = (root / "templates" / "partials" / "upload_preview.html").read_text(encoding="utf-8")
        self.assertIn("data-preview-format", tpl)
        self.assertIn("data-preview-title", tpl)
        self.assertIn("data-preview-author", tpl)
        self.assertIn("data-preview-filename", tpl)
