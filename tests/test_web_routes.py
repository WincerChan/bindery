...
class WebRoutesTests(unittest.TestCase):
    ...
    def test_ingest_template_has_single_file_input(self) -> None:
        ...
        self.assertIn("data-dedupe-keep-tokens", ingest)
        ...
        self.assertIn("syncPerFileDedupeChoices", ingest)
        ...
        self.assertIn("input.name = 'dedupe_keep_tokens'", ingest)
        self.assertIn("htmx:responseError", ingest)
        self.assertIn("status === 413", ingest)
        self.assertIn("dedupeModeInput.value = dedupeState.hasDuplicateHit ? 'normalize' : 'keep';", ingest)
    ...
    def test_ingest_preview_template_exposes_structured_preview_metadata(self) -> None:
        ...
        self.assertIn("data-dedupe-keep-checkbox", tpl)
        self.assertIn("请确认是否保留", tpl)
