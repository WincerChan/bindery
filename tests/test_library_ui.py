...
class LibraryUiTests(unittest.TestCase):
    ...
    def test_index_has_bulk_actions(self) -> None:
        ...
        self.assertIn("data-bulk-mode-toggle", index)
        self.assertIn("data-bulk-actions", index)
        self.assertIn("toggleBookSelectionById", index)
        self.assertIn("[data-book-detail-link]", index)
        self.assertIn("pointer-events-none", index)
        self.assertIn("[data-book-hover-actions]", index)
        self.assertIn("data-book-card", index)
    ...
    def test_index_has_pagination_controls(self) -> None:
        ...
        self.assertIn('name="per_page"', index)
        self.assertIn("data-per-page-input", index)
        self.assertIn("GRID_ROWS_PER_PAGE = 4", index)
        self.assertIn("syncPerPageForGrid", index)
