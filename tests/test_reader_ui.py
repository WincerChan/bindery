...
class ReaderUiTests(unittest.TestCase):
    def test_preview_template_has_toc_and_controls(self) -> None:
        ...
        self.assertIn('x-ref="frameC"', tpl)
        ...
        self.assertIn(":style=\"activeFrameKey === 'c' ? 'opacity:1;z-index:10;pointer-events:auto;' : 'opacity:0;z-index:0;pointer-events:none;'\"", tpl)
        ...
        self.assertIn("const resumeRequested = urlParams.get(\"resume\") === \"1\";", tpl)
        self.assertNotIn("const resumeRequested = urlParams.get(\"resume\") === \"0\";", tpl)
        self.assertIn("const initialProgress = {{ initial_progress|tojson }};", tpl)
        self.assertIn("fetch(`/book/${this.bookId}/progress`", tpl)
        self.assertIn("navigator.sendBeacon", tpl)
        ...
        self.assertIn("_viewportAnchorInset", tpl)
        self.assertIn("_prefetchSectionHtml", tpl)
        self.assertIn("_inactiveFrameKeys", tpl)
        self.assertIn("_findFrameWithSection", tpl)
        ...
        self.assertIn("onMobileTap", tpl)
        self.assertIn("_onFrameTap", tpl)
        self.assertIn("_isMobileViewport", tpl)
        self.assertIn("doc.addEventListener(\"pointerup\", (evt) => this._onFrameTap(evt, frameKey));", tpl)
        self.assertIn("手机左侧翻页", tpl)
        self.assertIn("手机右侧翻页", tpl)
        self.assertIn("md:hidden absolute inset-y-0 left-0 z-30", tpl)
        self.assertIn("md:hidden absolute inset-y-0 right-0 z-30", tpl)
        self.assertIn("touch-manipulation", tpl)
        self.assertIn("window.matchMedia(\"(max-width: 767px)\")", tpl)
        self.assertIn("@media (max-width: 767px){ #bindery-reader-scroller{overflow-x:hidden!important;overscroll-behavior-x:none!important;touch-action:pan-y!important;}}", tpl)
        self.assertIn("frame.src = contentUrl", tpl)
        self.assertIn("tocEntries: sectionsData.map(", tpl)
        self.assertIn("_sectionContentUrl(index)", tpl)
        ...
        self.assertIn("--bindery-page-tail-offset", tpl)
        self.assertIn("bindery-reader-tail", tpl)
        self.assertIn("const pageFloatEpsilon = 0.5;", tpl)
        ...
        self.assertIn("#bindery-reader-tail{display:block!important;position:absolute!important;left:calc(100% + var(--bindery-page-tail-offset,0px))!important", tpl)
        self.assertIn("pages.appendChild(tail);", tpl)
        self.assertNotIn("scroller.appendChild(tail);", tpl)
        self.assertIn("root.style.setProperty(\"--bindery-page-tail-offset\", \"0px\");", tpl)
        self.assertIn("root.style.setProperty(\"--bindery-page-tail-pad\", \"0px\");", tpl)
        self.assertIn("root.style.setProperty(\"--bindery-page-tail-offset\", rawMaxLeft + \"px\");", tpl)
        self.assertIn("doc.documentElement.style.setProperty(\"--bindery-page-tail-offset\", \"0px\");", tpl)
        self.assertIn("doc.documentElement.style.setProperty(\"--bindery-page-tail-offset\", rawMaxLeft + \"px\");", tpl)
        self.assertIn("const wasAtEnd = !!(previousMetrics && previousMetrics.maxLeft - scroller.scrollLeft <= pageFloatEpsilon);", tpl)
        self.assertIn("scroller.scrollTo({ left: metrics.maxLeft, top: -1, behavior: \"auto\" });", tpl)
        self.assertIn("Math.ceil((maxLeft - pageFloatEpsilon) / step) + 1", tpl)
        self.assertIn("Math.floor((scroller.scrollLeft + pageFloatEpsilon) / metrics.step) + 1", tpl)
        self.assertIn("if (metrics.maxLeft - scroller.scrollLeft <= pageFloatEpsilon)", tpl)
        self.assertIn("return Math.max(1, pageCount);", tpl)
        self.assertNotIn("Math.floor(maxLeft / step) + 1", tpl)
        self.assertNotIn("Math.floor((scroller.scrollLeft + 1) / metrics.step) + 1", tpl)
        self.assertNotIn("margin-right:var(--bindery-page-tail-pad,0px)!important", tpl)
        self.assertNotIn("display:flex!important;flex-wrap:nowrap!important;align-items:stretch!important", tpl)
        self.assertNotIn("flex:0 0 100%!important", tpl)
        self.assertNotIn("flex:0 0 var(--bindery-page-tail-pad,0px)!important", tpl)
        self.assertIn("this._scrollByViewportStep(-1)", tpl)
        self.assertIn("this._scrollByViewportStep(1)", tpl)
        self.assertIn("_scrollByViewportStep(direction)", tpl)
        ...
        self.assertIn("let preserveReverseSection = false;", tpl)
        self.assertIn("preserveReverseSection = true;", tpl)
        self.assertIn("this._frameState[key].section !== reverseIndex", tpl)
        ...
        self.assertIn("const desiredInset = this._viewportAnchorInset(scrollerRect);", tpl)
        self.assertIn("const anchorLeft = Math.max(0, scroller.scrollLeft + rect.left - scrollerRect.left);", tpl)
        self.assertIn("const desiredLeft = Math.max(0, anchorLeft - desiredInset);", tpl)
        self.assertIn("const rawPage = Math.floor((anchorLeft + pageFloatEpsilon) / metrics.step) + 1;", tpl)
        self.assertNotIn("const rawPage = Math.floor((desiredLeft + pageFloatEpsilon) / metrics.step) + 1;", tpl)
        self.assertNotIn("const deltaX = rect.left - scrollerRect.left - 7;", tpl)
        self.assertNotIn("scroller.scrollLeft + deltaX", tpl)
        ...
        self.assertIn("_computeTailPad(maxLeft, step)", tpl)
        self.assertIn("_attachLayoutObserver(frameKey)", tpl)
        self.assertIn("new ResizeObserver(() => {", tpl)
